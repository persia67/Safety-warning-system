...
const DashboardStats: React.FC<DashboardStatsProps> = ({ violations = [], rewards = [], mode, language = 'fa' }) => {
  ...
  if (mode === 'REWARD') {
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
        {cards.map((card, index) => (
          <div key={index} className={`p-6 rounded-2xl border ${card.bg} ${card.border} shadow-sm transition-transform hover:-translate-y-1`}>
